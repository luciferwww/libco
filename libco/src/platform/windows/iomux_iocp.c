...
static co_error_t co_wait_io_async(co_socket_t fd, co_io_op_t op_type, 
    ...
    // Initialize timeout state. Full Windows IOCP timeout handling is described below.
    current->timed_out = false;
    current->io_waiting = false;
    // TODO(Windows I/O timeout): IOCP timeout handling is significantly more
    // complex than epoll and requires:
    //   1. registering the coroutine in the timer heap
    //   2. letting the timer handler mark timed_out = true and resume it
    //   3. detecting timed_out in co_wait_io_async and then:
    //      a. calling CancelIoEx((HANDLE)fd, &wait_ctx.overlapped)
    //      b. performing a second co_context_swap to wait for the IOCP cancel
    //         completion packet (ERROR_OPERATION_ABORTED)
    //      c. returning CO_ERROR_TIMEOUT after the second resume
    //   Current behavior: timeout_ms is ignored and co_wait_io_async waits
    ...
    // Once resumed, the I/O has completed
